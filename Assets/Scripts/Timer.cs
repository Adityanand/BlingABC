...
public class Timer : MonoBehaviour {
	...
	void Update () {
       //if (UsingTimer)
            SetUIText();
        if (OneStar.GetComponent<Image>().enabled == true)
        {
            Stars = 1;
            Star11.GetComponent<Image>().enabled = true;
        }
        if (TwoStar.GetComponent<Image>().enabled == true)
        {
            Stars = 2;
    ...
    public void SetUIText()
    {
        ...
        if (TimeLeft<=0)
        {
            ...
            if (TimeLeft <= 0)
            {
                ...
                if (Stars <= 1)
                {
                    StarCount.text = ("You completed " + Stars + " match in 30 seconds.");
                }
                else
                {
                    StarCount.text = ("You completed " + Stars + " matches in 30 seconds.");
                }
                Camera.GetComponent<UIElementDragger1>().enabled = false;
                GameObject Gameobject = GameObject.Find("Main Camera");
                Gameobject.GetComponent<AudioSource>().enabled = false;
